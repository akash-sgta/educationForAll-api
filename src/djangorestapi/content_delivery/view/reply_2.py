...
from content_delivery.models import Coordinator, ReplyToReply
from content_delivery.serializer import ReplyToReply_Serializer
...
class Reply_2_View(APIView):
    ...
    def get(self, request, pk=None):  # TODO : Users can see all replies indivisually
        ...
        if pk not in (None, ""):
            ...
            if isAuthorizedUSER[0] == False:
                ...
                try:
                    reply_ref = ReplyToReply.objects.get(pk=int(pk))
            ...
            data["success"] = False
            data["message"] = {"METHOD": "GET", "URL_FORMAT": "/api/content/replyD/<parent_reply_id>"}
            return Response(data=data, status=status.HTTP_400_BAD_REQUEST)
    ...
    def put(self, request, pk=None):  # TODO : Users can edit own replies
        ...
        if pk not in (None, ""):
            ...
            if isAuthorizedUSER[0] == False:
                ...
                try:
                    reply_ref = ReplyToReply.objects.get(pk=int(pk), user_ref=isAuthorizedUSER[1])
                except ReplyToReply.DoesNotExist:
                    data["success"] = False
            ...
            data["success"] = False
            data["message"] = {"METHOD": "PUT", "URL_FORMAT": "/api/content/replyD/<deep_reply_id>"}
            return Response(data=data, status=status.HTTP_400_BAD_REQUEST)
    ...
    def delete(self, request, pk=None):  # TODO : Users can delete own replies
        ...
        if pk not in (None, ""):
            ...
            data["success"] = False
            data["message"] = {"METHOD": "DELETE", "URL_FORMAT": "/api/content/replyD/<deep_reply_id>"}
            return Response(data=data, status=status.HTTP_400_BAD_REQUEST)
    ...
    def options(self, request, pk=None):
        ...
        temp["POST"] = {"reply_ref": "Number [FK]", "body": "String : unl"}
        temp["GET"] = None
        temp["PUT"] = {"reply_ref": "Number [FK]", "body": "String : unl"}
        temp["DELETE"] = None
