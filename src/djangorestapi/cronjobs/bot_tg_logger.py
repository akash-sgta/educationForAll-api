        ...
        try:
            ...
            while(not self.stop_thread):
                try:
                    ...
                    print(f"[T] No of queries\t:\t{len(all_updates)}")
                        ...
                        for current_update in all_updates:
                            try:
                                ...
                                if(first_chat_text in ('/hello', '/start')):
                                    ...
                                    print(f'[T] HELLO | ID\t:\t{first_chat_id} | NAME\t:\t{first_chat_name}')
                                    ...
                                    try:
                                        ...
                                        self.send_message(first_chat_id, text)
                                        new_offset = first_update_id + 1
                                        print(f'[T] LOGOUT | ID\t:\t{first_chat_id} | NAME\t:\t{first_chat_name}')
                                    ...
                                    self.send_message(first_chat_id, text)
                                    new_offset = first_update_id + 1
                                    print(f'[T] LOGIN_1 | ID\t:\t{first_chat_id} | NAME\t:\t{first_chat_name}')

                                else:
                                    if(first_chat_id not in IN_TRANSIT):
                                        text = 'Invalid Command.\nTry\t:\t/hello'
                                        print(f'[T] OTHERS | ID\t:\t{first_chat_id} | NAME\t:\t{first_chat_name}')
                                        ...
                                        print(f'[T] LOGIN_2 | ID\t:\t{first_chat_id} | NAME\t:\t{first_chat_name}')

                                    self.send_message(first_chat_id, text)
                                    new_offset = first_update_id + 1
                            
                            except Exception as ex:
                                print(f'[T] Exception Occured\t:\t{str(ex)}')
                                new_offset = first_update_id + 1
                            else:
                                pass
