    ...
    def get(self, request, word=None):
        ...
        if(word in (None, "")):
            ...
            if(isAuthorizedAPI[0]):
                api_ref = Api_Token_Table.objects.get(pk = isAuthorizedAPI[1])
        ...
        if(word in (None, "")):
            ...
            if(word.lower() == 'signup'):
                ...
                try:
                    ...
                    try:
                        api_ref = Api_Token_Table(user_name = f"{temp['user_f_name'][0]} {temp['user_l_name'][0]}",
                                                    user_email = f"{temp['user_email'][0].lower()}",
                                                    user_password = data,
                                                    user_key_private = random_generator(),
                                                    api_endpoint = temp['api_endpoint'][0])
                        api_ref.save()
                    except Exception as ex:
                        print("-"*25)
                        ...
                        data_returned['endpoint'] = api_ref.api_endpoint
                        data_returned['pinned'] = Notification_Serializer(Notification.objects.filter(prime=True).order_by('-notification_id'), many=True).data
                        return cookie.set_authentication_info(request=request, file_path='auth_prime/index.html', data=data_returned, pk=api_ref.pk)
                ...
                try:
                    ...
                    try:
                        api_ref = Api_Token_Table.objects.get(user_email = f"{temp['user_email'][0].lower()}",
                                                                 user_password = data)
