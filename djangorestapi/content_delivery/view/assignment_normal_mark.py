...
from content_delivery.models import Assignment, Coordinator
...
class Assignment_Mark_View(APIView):
        ...
        if pk_a not in (None, "") and pk_s not in (None, ""):
            ...
            if isAuthorizedUSER[0] == False:
                ...
                try:
                    ...
                    data["success"] = False
                    data["message"] = {"URL_FORMAT": "/api/content/assignment_mark/<assignent_id>/<submission_id>"}
            ...
            data["success"] = False
            data["message"] = {"METHOD": "GET", "URL_FORMAT": "/api/content/assignment_mark/<assignent_id>/<submission_id>"}
            return Response(data=data, status=status.HTTP_400_BAD_REQUEST)
    ...
    def put(self, request, pk_a=None, pk_s=None):  # TODO : Only Coordinator can change marks
        ...
        if pk_a not in (None, "") or pk_s not in (None, ""):  # TODO : Both have to be populated
            ...
            if isAuthorizedUSER[0] == False:
                ...
                try:
                    ...
                    if "marks" not in request.data.keys():
                        ...
                        try:
                            submission_ref = Submission.objects.get(assignment_ref=pk_a, pk=pk_s)
                        except Submission.DoesNotExist:
                            data["success"] = False
                            data["message"] = "INVALID_ASSIGNMENT_SUBMISSION_PAIR"
                            return Response(data=data, status=status.HTTP_404_NOT_FOUND)
            ...
            data["success"] = False
            data["message"] = {"METHOD": "PUT", "URL_FORMAT": "/api/content/assignment_mark/<assignent_id>/<submission_id>"}
            return Response(data=data, status=status.HTTP_400_BAD_REQUEST)
